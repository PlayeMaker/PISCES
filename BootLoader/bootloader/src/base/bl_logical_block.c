 ...
 *      05.02.00 | 27/03/2016 | beibei.xing      | N/A | Boot050015
 *      05.03.00 | 10/09/2019 | lele.liu         | N/A | Boot050021
 *      05.07.00 | 25/06/2021 | yatai.liu        | N/A | Boot050707
 *
 *****************************************************************************/
#include "bl_common.h"
 ...
 *****************************************************************************/
bl_HashContext_t gs_HNSHashContext;
/*****************************************************************************
...
/**************************************************************************//**
 *
 *  \details Check requested download address and size whether in valid block.
 *
 *  \param[in]  address - the address of request download.
 *  \param[in]  size    - the size of request download.
 *
 *  \retval BL_ERR_OK - the requested download information is valid.
 *  \retval BL_ERR_NOT_OK - the requested download information is invalid.
 *
 *  \since  V5.7.0
 *
 *****************************************************************************/
bl_Return_t Lbm_CheckDownloadBlock(bl_Address_t address,
                                    bl_Size_t size)
{
    bl_Return_t ret;
    bl_u16_t foundId;
...
bl_Return_t Lbm_Erase(const bl_DownContext_t *dct)
{
    ...
    do
    {
        ...
        if (ret != BL_ERR_OK)
        {
            ret = BL_ERR_MEMORYDRIVER_UNINIT;
            break;
        }
                        ...
                        const bl_Buffer_t *value)
{
    ...
    do
    {
        ...
        if (LBM_IS_A_DRIVER(block))
        {
            ...
            if (ret == BL_ERR_OK)
            {
                Rte_SetDownStatus(&g_DownContext, RTE_SYSFLAG_INIT_FLASH_DRIVER);
            }
            else
            {
                break;
...
static void _Lbm_ReadAndUpdateStatus(const bl_LogicalBlock_t *lb)
{
    ...
    bl_u16_t status = LBM_STATUS_INVALID;
...
{
    bl_Return_t ret;
    bl_Buffer_t hash[SECM_SHA256_HASH_LENGTH];
#if (LBM_FUN_AUTO_CALCULATE_HASH == BL_FUN_ON)
    const bl_LogicalBlock_t *block;
#endif
    ...
    if (BL_ERR_OK == ret)
    {
        ...
        ret = SecM_FinishHash(&gs_HNSHashContext, SECM_SHA256_HASH_LENGTH, hash);
...
static bl_Return_t _Lbm_CalculateHashByDct(const bl_DownContext_t *dct)
{
    ...
    bl_u32_t timeout = 0;
    bl_Return_t ret = BL_ERR_NOT_OK;
    bl_Buffer_t seg_startadd_size[8]={0};/*calculate a segment's start address and size for NIO.by yanhui*/
    bl_u32_t count = 0;
    bl_Buffer_t Service31_78_cal_data[8] = {0x03, 0x7F, 0x31, 0x78, 0xAA, 0xAA, 0xAA, 0xAA};
    const bl_Buffer_t *Service31_78_cal = Service31_78_cal_data;
      
    SecM_InitHashContext(HNS_HASH_ALGORITHM_ID, &gs_HNSHashContext);
    SecM_StartHash(&gs_HNSHashContext);
    ...
    for (i = 0; i < dct->segIndex; i++)
    {
        ...
        Bl_McuToBe32(seg_startadd_size,calAddr);
        Bl_McuToBe32(&seg_startadd_size[4],calSize);
        (void)SecM_UpdateHash(&gs_HNSHashContext,8,seg_startadd_size);
        ...
        do
        {
            ...
            if (calSize > HNS_DECOMPRESSED_DATA_BUFFER_SIZE)
            {
                readSize = HNS_DECOMPRESSED_DATA_BUFFER_SIZE;
            }
            else
            {
                readSize = calSize;
            ...
            ret = SecM_UpdateHash(&gs_HNSHashContext,readSize,g_HashCalculation);
#if(RTE_FUN_WATCHDOG == BL_FUN_ON)
            Wdg_Feed();
#endif
            if (ret != BL_ERR_OK)
...
static bl_u16_t _Lbm_LookForMemId(const bl_LogicalBlock_t *lb)
{
    ...
    if(lb->index == 0)
    {
        addr = FLASHIF_DRIVER_STARTADDRESS;
    }
...
/**************************************************************************//**
 *
 *  \details clear the flash driver data in RAM.
 *
 *  \return none
 *
 *  \since
 *
 *****************************************************************************/
void Lbm_ClearFlashDriver(void)
{
    bl_u16_t i;
    const bl_LogicalBlock_t *block;
    ...
    {
        block = LBM_GET_LBPTR(i);
        if (LBM_TYPE_DRIVER == block->type)
        {
            Bl_MemSet((bl_Buffer_t *)(LBM_DRIVER_ADDRESS), 0x00, LBM_DRIVER_size);
            break;
        }
    }
}
