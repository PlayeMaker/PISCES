 ...
 *      05.03.00 | 27/03/2017 | beibei.xing      | N/A | Boot050017
 *      05.04.00 | 27/03/2018 | beibei.xing      | N/A | Boot050018
 *
******************************************************************************/
#include "bl_common.h"
#include "bl_rte.h"
...
#define FLASHIF_STATE_INITED   0x1000
#define FLASHIF_STATE_UNINIT   0x0000

#define UDS_SERVICE_ID_31     (0x31u)

/*****************************************************************************
 *  Internal Variable Definitions
 *****************************************************************************/
static bl_u16_t gs_FlsIfInitSt = FLASHIF_STATE_UNINIT;
 ...
 *****************************************************************************/
bl_u32_t erasereturn=0;
bl_u32_t writereturn=0;
/*****************************************************************************
...
bl_Return_t Flsif_Init(void)
{
    ...
    bl_u32_t checkValue;
    bl_u32_t Address = 0;
    if(gs_FlsIfInitSt == FLASHIF_STATE_INITED)
    {
        return BL_ERR_OK;
    }
    ...
    if (FLASHIF_DRIVER_MAGIC_VALUE == checkValue)
    {
        ...
        {
            Address = *(bl_u32_t *)(FLASHIF_DRIVER_STARTADDRESS + 0x0C);
            FLASHIF_INIT(&param);
            ret = param.errCode;
            if(BL_ERR_OK == ret)
            {
                gs_FlsIfInitSt = FLASHIF_STATE_INITED;
            }
        }
    }
...
bl_Return_t Flsif_DeInit(void)
{
    ...
    {
        gs_FlsIfInitSt = FLASHIF_STATE_UNINIT;
    }
...
#endif
    erasereturn++;
    if(serviceid == UDS_SERVICE_ID_31)
    {
        if(erasereturn>80)
        {
            _Adpt_TimeOutCallBack(UDS_SERVICE_ID_31);
            erasereturn = 0;
        }
    }
