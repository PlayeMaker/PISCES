 ...
 *      05.02.00 | 27/03/2016 | beibei.xing      | N/A | Boot050006
 *      05.03.00 | 27/03/2017 | beibei.xing      | N/A | Boot050007
 *      05.04.00 | 27/03/2018 | beibei.xing      | N/A | Boot050008
 *      05.05.00 | 27/08/2018 | beibei.xing      | N/A | Boot050009
 *      05.07.00 | 25/06/2021 | yatai.liu        | N/A | Boot050709
 *
 *****************************************************************************/
#include "bl_common.h"
#include "bl_rte.h"
...
#define DCM_31SERVICE_ID                    (0x0031u)
#define DCM_31SERVICE_START_ROUTINE_SUBID   (0x0001u)
#define DCM_31SERVICE_RID_INDEX             (0x02u)
#define DCM_SERVICE_SID_FLAG                (0x10u)
#define DCM_31SERVICE_SUBID_FLAG            (0x20u)
#define DCM_31SERVICE_INVALID_VALUE         (0x01u)
...
#define DCM_CLEARSUPPRESSPOSBIT             (0x7Fu)
...
static bl_BufferSize_t _Dcm_MakeSid(bl_Buffer_t *buffer, bl_u16_t *id);
/** \brief Make subfunction id.*/
static bl_BufferSize_t _Dcm_MakeSubId(bl_Buffer_t *buffer, bl_u16_t *id);
/** \brief Make data identifier id.*/
static bl_BufferSize_t _Dcm_MakeDid(bl_Buffer_t *buffer, bl_u16_t *id);
...
/** \brief 31 service flag.*/
static bl_u8_t gs_Dcm31ServiceFlag = DCM_31SERVICE_INVALID_VALUE;
...
{
    DCM_TYPE_SERVICE_DID, 0x31u,0x31u, &_Dcm_MakeDid,
};
...
/**************************************************************************//**
 *
 *  \details Dcm clear suspress positive response bit.
 *
 *  \note
 *
 *  \since  V5.1.0
 *
 *****************************************************************************/
void Dcm_AllowPositiveResp(void)
{
    DCM_ALLOW_POSITIVE_RESPONSE();
}
...
static void _Dcm_MainFunction(void)
{
    ...
    do
    {
        ...
        /* check 31 rid*/
        if (DCM_SERVICE_SID_FLAG == gs_Dcm31ServiceFlag)
        {
            const bl_DcmServiceHeader_t *subheader;
            const bl_DcmService_t *sublist;
            bl_u16_t rid = 0;

            gs_Dcm31ServiceFlag |= DCM_31SERVICE_SUBID_FLAG;
            sublist = _Dcm_LookUpTable(DCM_31SERVICE_START_ROUTINE_SUBID, list->number, list->serviceList);
            if ((BL_NULL_PTR != sublist->serviceList))
            {
                subheader = sublist->header;
                subheader->MakeId(&buffer[DCM_31SERVICE_RID_INDEX], &rid);

                sublist = _Dcm_LookUpTable(rid, sublist->number, sublist->serviceList);
                if(BL_NULL_PTR == sublist)
                {
                    resCode = subheader->resCode;
                    break;
                }

                ret = _Dcm_CheckSessionFlag(sublist->flag);
                if(ret != BL_ERR_OK)
                {
                    resCode = subheader->sessionResCode;
                    break;
                }

                ret = _Dcm_CheckSecAccFlag(sublist->flag);
                if(ret != BL_ERR_OK)
                {
        ...
        /* check whether sid is 0x31 */
        if ((DCM_SERVICE_SID_FLAG & gs_Dcm31ServiceFlag) == 0)
        {
            gs_Dcm31ServiceFlag |= DCM_SERVICE_SID_FLAG;
            if (DCM_31SERVICE_ID == id)
            {
                gs_Dcm31ServiceFlag &= ~DCM_31SERVICE_INVALID_VALUE;
            }
        }
        ...
        /*check whether id is supported in current session.*/
...
static bl_BufferSize_t _Dcm_MakeSubId(bl_Buffer_t *buffer, bl_u16_t *id)
{
    ...
    if((buffer[0] & DCM_SUPPRESSPOSRSPMSGINDICATIONBITMASK) != 0u)
    {
        ...
        buffer[0] &= DCM_CLEARSUPPRESSPOSBIT;
    }
    else
    {
        DCM_ALLOW_POSITIVE_RESPONSE();
