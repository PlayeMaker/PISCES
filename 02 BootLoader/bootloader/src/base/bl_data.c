 ...
 *      05.02.00 | 27/03/2016 | beibei.xing      | N/A | Boot050011
 *
 *****************************************************************************/
#include "bl_common.h"
#include "bl_memory.h"
...
bl_Return_t Dm_ClearData(bl_u16_t did)
{
    ...
    if (DM_IS_VALID_ID(did))
    {
        ...
        if ((DM_DATA_TYPE_READ_AND_WRITE == data->type) && (data->defData != BL_NULL_PTR) && (data->size > 0UL))
        {
            tmpBuf = data->defData;
            ...
            if ((data->localAddr) == 0x3FFF0)
            {
            	ret = Mem_Erase(data->memId,
                          data->localAddr,
                          data->size);
            }
            else
